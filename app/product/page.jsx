      ...
      <section className="container mx-auto p-10 md:py-12 px-0 md:p-8 md:px-0">
        <section className="p-5 md:p-0 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-10 items-start ">
           {productData.map((data) => (
              <section key={data?.id} className="p-5 py-10 bg-purple-50 text-center transform duration-500 hover:-translate-y-2 cursor-pointer">
              <img
                src={data.image}
                alt=""
              />
              ...
              <h1 className="text-3xl my-5">{data.product_name}</h1>
              <p className="mb-5">
                {data.description}
              </p>
              ...
              <button className="p-2 px-6 bg-purple-500 text-white rounded-md hover:bg-purple-600">
                <Link href={`/product/${data?.id}`}>Product details</Link>
              </button>
            </section>
           ))}
